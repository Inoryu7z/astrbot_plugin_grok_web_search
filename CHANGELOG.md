...
### Fixed
- **自定义提供商路径 content 级错误检测**：某些 API 中转商在请求不合法时返回 HTTP 200 + 正常 JSON，但 content 字段为错误信息（如 Invalid request. Please update your client.），此前被误判为搜索成功并阻止了继续 fallback。现在对返回内容进行错误模式检测，匹配到错误内容时自动切换到下一个提供商
- **内置提供商路径错误模式列表补充**：新增 invalid request 和 please update your client 两个错误模式

## [1.4.0] - 2026-04-29

### Added
- **豆包 Bot API 网页抓取**：新增 `doubao_bot_fetch()` 函数，通过 `/api/v3/bots/chat/completions` 调用带网页解析插件的零代码应用进行网页抓取
  - 支持从响应的 `references` 提取 `cover_image` 作为图片 URL
  - 支持从文本内容正则匹配图片 URL（.jpg/.png/.gif/.webp 等）
  - 返回结果包含 `image_urls` 字段
- **`grok_web_fetch` 工具路由优化**：优先使用配置了 `fetch_bot_id` 的豆包提供商进行网页抓取，回退到 Grok 提供商
  - 未配置 `fetch_bot_id` 的豆包提供商跳过（不报错）
  - 有 `fetch_bot_id` 的豆包提供商排在最前面
- **网页图片自动发送**：网页抓取结果中的图片自动下载并发送给用户，受 `doubao_max_images` 配置限制
- **新增 `fetch_bot_id` 配置项**（providers 模板）：豆包零代码应用ID，需在豆包控制台创建带网页解析插件的应用
- **新增 `doubao_max_images` 配置项**（全局，默认3，0-10）：控制网页解析时最大发送图片数量，0 表示不发送图片
- **Skill 脚本支持 Bot API**：`grok_search.py` 新增 `_request_doubao_bot_fetch()` 和 `_parse_doubao_bot_result()` 函数
- **`grok_download_file` LLM Tool**：文件下载并发送工具，支持图片和文档
  - 下载指定 URL 的文件到本地并发送给用户
  - 图片：直接发送图片消息（`file_image` → 回退 `fromBytes`）
  - 文档（PDF/Word/Excel 等）：直接发送文件消息
  - 返回本地路径供后续工具引用
  - 支持代理、超时、20MB 大小限制
  - 自动检测 Content-Type 修正文件类型

### Changed
- `grok_web_fetch` 工具描述更新：说明优先使用豆包网页解析应用，支持图片提取
- `_get_custom_provider_pool()` 现在读取每个提供商的 `fetch_bot_id` 字段
- `_run_custom_provider_fetch()` 豆包提供商 + `fetch_bot_id` 时调用 `doubao_bot_fetch()`，否则回退到 Grok
