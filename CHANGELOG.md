...
* `grok_web_search` 的说明写明本工具用于检索互联网信息：query 会作为问题交给联网大模型作答，需用完整自然语言表达，而非关键词堆砌。
* 找图片的场景限定为「互联网上的图片」；图库里已经存过的照片（自己以前拍的、收进来的别人的照片）改由衣橱的取图工具处理，不走本搜索流程。
* `grok_download_file` 同步该边界，并合并掉与适用场景重复的条目。
