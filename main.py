...
from .api.doubao_responses import (
    doubao_responses_search,
    doubao_bot_fetch,
    is_doubao_provider,
)
...
class GrokSearchPlugin(Star):
    ...
    def _unregister_disabled_tools(self):
        ...
        if self.config.get("enable_skill", False):
            ...
            _llm_tools_registry.remove_func("grok_download_file")
            logger.info(
                f"[{PLUGIN_NAME}] Skill 已启用，已卸载 grok_web_search、grok_web_fetch 和 grok_download_file 工具"
            )
            return
        ...
        *,
        ...
        if is_doubao_provider(base_url):
            ...
            result = await doubao_responses_search(
                ...
                reasoning_effort=str(provider_cfg.get("reasoning_effort") or "").strip()
                or None,
            )
    ...
    async def _run_custom_provider_fetch(
        ...
        *,
        ...
        base_url = str(provider_cfg.get("base_url") or "")
        fetch_bot_id = str(provider_cfg.get("fetch_bot_id") or "").strip()

        if is_doubao_provider(base_url) and fetch_bot_id:
            result = await doubao_bot_fetch(
                url=url,
                base_url=base_url,
                api_key=str(provider_cfg.get("api_key") or ""),
                bot_id=fetch_bot_id,
                ...
                retryable_status_codes=set(
                    self.config.get("retryable_status_codes", [429, 500, 502, 503, 504])
                ),
                proxy=proxy,
            )
        else:
        ...
        for provider_cfg in providers:
            ...
            if result.get("ok"):
                ...
                if is_error_content:
                    ...
                    provider_errors.append(f"{provider_name}: {err_msg}")
                    logger.warning(
                        f"[{PLUGIN_NAME}] {provider_name} {err_msg}，切换到下一个提供商"
                    )
                    continue
                if provider_index != 1:
                    logger.info(
    ...
    async def grok_tool(
        ...
        - 找互联网上的图片/素材：先搜索拿到图片 URL，再调 grok_download_file 下载发送
...
- 用户想要的是图库里已经存过的照片（bot 自己以前的、或收进来的别人的照片 / cos 照）→ 用图库检索工具 search_wardrobe_image；本工具接触不到本地图库
        ...
        query 编写原则（本工具原理：把 query 当作问题交给一个联网的大模型去检索作答）：
        - 用完整的自然语言把意图说成一句能读懂的话，关键词堆砌会让联网模型误解意图
    ...
    async def grok_fetch_tool(self, event: AstrMessageEvent, url: str):
        """网页正文抓取工具。用于读取指定 URL 的网页主体内容，并借助联网能力将其转换为结构化 Markdown 返回。优先使用豆包网页解析应用（如已配置 fetch_bot_id），可自动提取并发送网页中的图片。

        适用场景：
        - 用户给出一个明确 URL，并要求"看看这个页面写了什么"
        - 需要提取网页中的正文、表格、列表、代码片段、公告、文章内容
        - 需要查看网页中的图片（需配置豆包网页解析应用）
        ...
        for p in providers:
            ...
            fetch_bot_id = str(p.get("fetch_bot_id") or "").strip()
            if is_doubao_provider(base_url) and fetch_bot_id:
                doubao_providers.append(p)
            else:
                grok_providers.append(p)

        ordered_providers = doubao_providers + grok_providers

        max_images = self.config.get("doubao_max_images", 3)
        provider_errors: list[str] = []
        for provider_cfg in ordered_providers:
            ...
            base_url = str(provider_cfg.get("base_url") or "")
            fetch_bot_id = str(provider_cfg.get("fetch_bot_id") or "").strip()
            is_doubao_fetch = is_doubao_provider(base_url) and fetch_bot_id

            if is_doubao_provider(base_url) and not fetch_bot_id:
                provider_errors.append(
                    f"{provider_name}: 豆包提供商未配置 fetch_bot_id，跳过网页抓取"
                )
                logger.info(
                    f"[{PLUGIN_NAME}] {provider_name} 为豆包提供商但未配置 fetch_bot_id，跳过网页抓取"
                )
                continue

            log_detail = (
                f"Bot API (bot_id={fetch_bot_id})"
                if is_doubao_fetch
                else f"model={provider_model}"
            )
            logger.info(
                f"[{PLUGIN_NAME}] 正在尝试 {provider_name} ({log_detail}) 执行网页抓取"
            )
            try:
            ...
            if result.get("ok"):
                ...
                if is_doubao_fetch:
                    provider_suffix += " (豆包网页解析)"

                image_urls = result.get("image_urls", [])
                if max_images > 0 and image_urls:
                    image_urls = image_urls[:max_images]
                    sent_count = 0
                    for img_url in image_urls:
                        try:
                            await event.send(MessageChain().image(img_url))
                            sent_count += 1
                        except Exception as e:
                            logger.warning(
                                f"[{PLUGIN_NAME}] 图片发送失败 ({img_url}): {e}"
                            )
                    if sent_count > 0:
                        logger.info(f"[{PLUGIN_NAME}] 已发送 {sent_count} 张网页图片")
    ...
    @filter.llm_tool(name="grok_download_file")
    async def grok_download_tool(
        self,
        event: AstrMessageEvent,
        url: str,
    ) -> str:
        """文件下载并发送工具。用于下载指定 URL 的图片或文档文件，自动发送给用户并返回本地路径。

        适用场景：
        - 用户要互联网上的图片：先用 grok_web_search 搜索获取图片 URL，再用本工具下载发送（图库里已有的照片不走本流程）
        - 用户给出一个直接文件链接，要求下载并发送
        - 需要将网络图片/文档下载到本地供后续代码执行使用

        使用原则：
        - 本工具是"下载文件并发送"，不是搜索工具也不是网页内容读取工具
        - 当任务核心是"搜索/查证/找资料"时，优先使用 grok_web_search
        - 当需要读取网页正文内容时，优先使用 grok_web_fetch
        - 仅当需要下载具体的文件（图片、PDF、文档等）并发送给用户时，才使用本工具

        Args:
            url(string): 要下载的文件 URL，必须是完整的 HTTP/HTTPS 地址，指向具体的文件（图片/PDF/文档等）
        """
        if not url or not url.startswith("http"):
            return "错误：请提供完整的 HTTP/HTTPS URL"
        ...
        plugin_data_dir = self._get_plugin_data_path()
        download_dir = plugin_data_dir / "downloads"
        ...
        while local_path.exists():
            ...
            local_path = download_dir / f"{stem}_{counter}{ext}"
            counter += 1

        logger.info(f"[{PLUGIN_NAME}] 正在下载文件: {url} -> {local_path}")
        ...
        try:
            ...
            if len(content) > max_size:
                return f"下载失败: 文件过大 ({len(content) / 1024 / 1024:.1f}MB > 20MB)"
            ...
            if not is_image and not is_document:
                if content_type.startswith("image/"):
                    ...
                    if ext == ".bin":
                        ...
                        if mime_ext:
                            local_path = local_path.with_suffix(mime_ext)
                elif any(
                    ct in content_type
                    for ct in ["pdf", "document", "spreadsheet", "zip", "octet-stream"]
                ):
                    is_document = True
            ...
            logger.info(
                f"[{PLUGIN_NAME}] 文件下载成功: {local_path} ({len(content) / 1024:.1f}KB)"
            )
            ...
            if is_image:
                try:
                    ...
                    file_sent = True
                    logger.info(f"[{PLUGIN_NAME}] 图片已发送: {local_path}")
                except Exception as e:
                    logger.warning(f"[{PLUGIN_NAME}] 图片发送失败，尝试 fromBytes: {e}")
                    try:
                        from astrbot.core.message.components import Image as ImgComp

                        await event.send(
                            event.chain_result([ImgComp.fromBytes(content)])
                        )
                        file_sent = True
                        logger.info(
                            f"[{PLUGIN_NAME}] 图片已发送(fromBytes): {local_path}"
                        )
                    except Exception as e2:
                        logger.warning(f"[{PLUGIN_NAME}] fromBytes 发送也失败: {e2}")
            elif is_document:
                try:
                    chain = [Comp.File(file=str(local_path), name=local_path.name)]
                    await event.send(event.chain_result(chain))
                    file_sent = True
                    logger.info(f"[{PLUGIN_NAME}] 文档已发送: {local_path}")
                except Exception as e:
                    logger.warning(f"[{PLUGIN_NAME}] 文档发送失败: {e}")

            if file_sent:
                return f"文件已下载并发送成功。本地路径: {local_path}"
            else:
                return f"文件已下载到本地但发送失败。本地路径: {local_path}\n可使用代码执行器读取该路径进行后续处理。"

        except asyncio.TimeoutError:
            return f"下载失败: 请求超时（{timeout}秒）"
        except aiohttp.ClientError as e:
            return f"下载失败: 网络错误 - {e}"
        except Exception as e:
            logger.error(f"[{PLUGIN_NAME}] 文件下载异常: {e}", exc_info=True)
            return f"下载失败: {e}"
