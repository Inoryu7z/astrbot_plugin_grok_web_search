...
async def grok_search(
    ...
    def _parse_sse_response(raw_text: str) -> dict[str, Any] | None:
        ...
        if not chunks:
            logger.warning(
                f"[grok] SSE: 未解析到任何有效 chunk，raw_text 前 500 字符: {raw_text[:500]}"
            )
            return None
        ...
        if error_info:
            ...
            logger.warning(f"[grok] SSE: API 返回错误: {error_msg}")
            return {
                "error": error_info,
                "error_message": error_msg,
                "choices": [{"message": {"content": f"API 错误: {error_msg}"}}],
                "model": model_name,
                "usage": usage_info,
            }

        final_content = merged_content or merged_reasoning
        if not merged_content and merged_reasoning:
            logger.warning(
                f"[grok] SSE: content 为空，回退使用 reasoning_content (len={len(merged_reasoning)})"
            )
        if not final_content:
            logger.warning(
                f"[grok] SSE: merged_content 和 merged_reasoning 均为空，"
                f"共 {len(chunks)} 个 chunk，第一个 chunk: "
                f"{json.dumps(chunks[0], ensure_ascii=False)[:500] if chunks else 'N/A'}"
            )
    ...
    try:
        ...
        if not choices or not isinstance(choices, list):
            ...
            logger.warning(
                f"[grok][DEBUG] 解析响应: content='{message[:50]}...', msg_keys={list(msg.keys())}"
            )
            if not message:
                reasoning = msg.get("reasoning_content") or ""
                logger.warning(
                    f"[grok][DEBUG] content为空, reasoning='{reasoning[:50]}...'"
                )
                if reasoning:
                    message = reasoning
                    logger.warning(
                        f"[grok] content 为空，回退使用 reasoning_content (len={len(reasoning)})"
                    )
                else:
                    logger.warning(
                        f"[grok] content 和 reasoning_content 均为空，完整 message 结构: "
                        f"{json.dumps(msg, ensure_ascii=False)[:2000]}"
                    )
                    parse_error = "choices[0].message.content 为空"
                    logger.warning(f"[grok][DEBUG] 已设置 parse_error={parse_error}")
