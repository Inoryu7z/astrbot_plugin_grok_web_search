...
def _request_doubao_bot_fetch(
    *,
    ...
    """通过豆包 Bot API (/api/v3/bots/chat/completions) 调用带网页解析插件的零代码应用"""
    api_url = f"{_normalize_base_url(base_url)}/api/v3/bots/chat/completions"
...
def _parse_doubao_bot_result(
    resp: dict[str, Any],
) -> tuple[str, list[dict[str, Any]], list[str]]:
    """解析豆包 Bot API 响应，提取 message 文本、references 和图片 URL"""
    message = ""
    ...
    for match in _re.finditer(img_url_pattern, message, _re.IGNORECASE):
        img_url = match.group().rstrip(".,;:!?\"'")
        if img_url and img_url not in image_urls:
            image_urls.append(img_url)

    return message, references, image_urls
...
def main() -> int:
    ...
    try:
        fetch_bot_id = ""
        if is_doubao and is_fetch_mode:
            providers_list = config.get("providers")
            if isinstance(providers_list, list) and providers_list:
                for prov in providers_list:
                    if not isinstance(prov, dict):
                        continue
                    prov_bot_id = str(prov.get("fetch_bot_id") or "").strip()
                    if prov_bot_id:
                        fetch_bot_id = prov_bot_id
                        break
            if not fetch_bot_id:
                fetch_bot_id = str(config.get("fetch_bot_id") or "").strip()

        if is_doubao and is_fetch_mode and fetch_bot_id:
            resp = _request_doubao_bot_fetch(
                base_url=base_url,
                api_key=api_key,
                bot_id=fetch_bot_id,
                url=fetch_url,
            ...
            is_bot_fetch = True
        elif is_doubao and not is_fetch_mode:
    ...
    bot_image_urls: list[str] = []

    if is_bot_fetch:
        message, api_citations, bot_image_urls = _parse_doubao_bot_result(resp)
    elif is_doubao:
    ...
    if is_fetch_mode:
        max_images = config.get("doubao_max_images", 3)
        out = {
            "ok": True,
            "fetch_url": fetch_url,
            "config_path": config_path,
            ...
            "content": message,
            "usage": resp.get("usage") or resp.get("bot_usage", {}) or {},
            "elapsed_ms": int((time.time() - started) * 1000),
        }
        if bot_image_urls and max_images > 0:
            out["image_urls"] = bot_image_urls[:max_images]
        sys.stdout.write(_compact_json(out))
        return 0
